#!/usr/bin/env bash
# Finetune SmolVLA on towel_fold01_step2 with chunk-wise DELTA JOINT actions.
#
#   ./train_smolvla_towel_fold01_step2_delta.sh
#
...
#
# The dataset is HyeonseokE/towel_fold01_step2_delta -- identical frames/videos to
# lhwdev/towel_fold01_step2, but with action stats recomputed in relative space.
...
#
# Baseline (absolute actions) for comparison: train_smolvla_towel_fold01_step2.sh
set -euo pipefail
...
lerobot-train \
  ...
  --dataset.repo_id=HyeonseokE/towel_fold01_step2_delta \
  ...
  --save_freq=$((STEPS + 1)) \
  --output_dir=outputs/train/smolvla_towel_fold01_step2_delta \
  --job_name=smolvla_towel_fold01_step2_delta \
  --policy.device=cuda \
  --policy.push_to_hub=true \
  --policy.repo_id=HyeonseokE/smolvla_towel_fold01_step2_delta \
  --policy.private=true \
  --wandb.enable=true \
  --wandb.project=smolvla_towel_fold01_step2_delta
