...
### C. SmolVLA — 3단계 파이프라인 (한 번 던지면 끝)

| # | 파일 | 비고 |
|---|---|---|
| 1 | **`main_job_pipeline.sbatch`** | **이것 하나만 던진다** |

```
stage 1  rank       3셀   seed 1000    ~71,350 step
stage 2  ablation2 12셀   seed 2000   ~262,000 step
stage 3  rq2       15셀   seed 1000   ~232,000 step
```

각 단계를 `--dependency=afterany` 로 앞 단계에 걸어 제출한다. `afterok` 이 아닌 이유는
셀 하나가 실패해도 뒤가 통째로 발이 묶이지 않게 하기 위해서다.

**동시 4학습.** 모든 단계가 `--array=...%2` (GPU 2장) 에 `PACK=2` (GPU당 2런) 다.
SmolVLA 두 런이 96GB 중 ~29GB 를 쓰고, `--cpus-per-task=18` 이 세션당 workers 8 을 덮는다.

> **2런 패킹이 1런보다 나은지는 아직 미확정이다.** 3런 패킹이 2026-09-07 에 `0.94 s/step`
> (3.19 step/s, 단독 3.14) 으로 이득이 0이었고, 같은 설정이 8/29 노드 한산 때는 `0.43`
> (2.2배 이득) 이었다. 2런은 그 중간이고 **첫 로그의 `updt_s` 가 0.64 미만이면 이득,
> 초과면 손해**다. 손해면 `PACK=1` 로 다시 던지면 된다.

데이터셋은 prefetch 가 아니라 **각 런이 시작할 때** 받는다. 파이프라인 도중에 수집이
끝나는 데이터셋은 거기에 도달하는 런이 알아서 집어간다.
