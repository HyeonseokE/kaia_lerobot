# SmolVLA on the ablation2 datasets -- shared body, sourced by
# cluster/train_ablation2.sbatch. No #SBATCH directives here: SLURM reads those only
# from the submitted wrapper.
#
# The caller must set A2_TASK and A2_COND before sourcing.
#
...
# These are not: the ablation2_* sets are still being collected condition by condition
# (A1 was not on the Hub when this was written), so there is no stable number to
# hardcode. STEPS is computed from the staged copy as
...
echo "=== ablation2 $COND / $TASK: $NAME  dataset=$DATASET  frames=$SRC_FRAMES  steps=$STEPS (50 epochs, batch 64, seed $SEED) ==="
nvidia-smi
...
apptainer exec --nv "$IMAGE" \
  lerobot-train \
    ...
    --wandb.enable=true \
    --wandb.project=smolvla_ablation2
