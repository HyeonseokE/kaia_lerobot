# SmolVLA on the benchmark-table OURS datasets -- shared body, sourced by
# cluster/train_bench_ours.sbatch. No #SBATCH directives here: SLURM reads those only
# from the submitted wrapper.
#
# The caller must set OURS_TASK (the task name) before sourcing.
#
# THE STEP BUDGET IS DERIVED AT RUNTIME, not from a table.
#
# Every other body in this directory hardcodes each dataset's frame count and refuses to
# train when the staged copy disagrees. That works when the data is already collected.
# These are not: several of the ours_* sets were still being recorded when this was
# written, so there is no number to hardcode. STEPS is computed from the staged copy as
# floor(frames/64) * 50 -- the same convention, just resolved later.
#
# What is lost is the cross-check that caught a re-upload. What replaces it is the
# .dataset_frames stamp in the run dir: a rerun against a changed dataset is still
# refused, it just reports the change instead of predicting it. The derived numbers are
# printed before training so the log records what a run actually got.
...
TASK="${OURS_TASK:?OURS_TASK not set. Source this from cluster/train_bench_ours.sbatch.}"
EPOCHS="${EPOCHS:-50}"
...
#
# This is the Ours arm, so the middle field is "ours". Note this is NOT the same as
...
NAME="smolvla_${TASK}_ours_${SEED}_10fps"

DS_SRC="$HOME/datasets/$DS"
[ -f "$DS_SRC/meta/info.json" ] || { echo "FATAL: $DS_SRC not found. Run cluster/main_job_bench_ours.sbatch first."; exit 1; }
...
echo "=== benchmark Ours / $TASK: $NAME  dataset=$DATASET  frames=$SRC_FRAMES  steps=$STEPS (50 epochs, batch 64, seed $SEED) ==="
nvidia-smi
...
apptainer exec --nv "$IMAGE" \
  lerobot-train \
    ...
    --wandb.enable=true \
    --wandb.project=smolvla_bench_ours
